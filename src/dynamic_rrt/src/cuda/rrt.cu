...
__device__ bool isGoal(float x, float y, float goalX, float goalY) {
	float threshold = 0.3f / 0.05f; //  threshold
...
__device__ TreeNode sampleFreeSpace(OccupancyGrid* grid, int iter, float startX, float startY, float startYaw) {
    ...
    thrust::uniform_real_distribution<float> dist01(0.5f / 0.05f, 8.0f / 0.05f);
    thrust::uniform_real_distribution<float> dist02(-2.0f / 0.05f, 2.0f / 0.05f);

    int maxAttempts = 1000;

    for (int attempt = 0; attempt < maxAttempts; attempt++) {
        float forward = dist01(rng);
        float lateral = dist02(rng);
        float x = startX + forward * cosf(startYaw) - lateral * sinf(startYaw);
        float y = startY + forward * sinf(startYaw) + lateral * cosf(startYaw);
...
__global__ void kernRRT(
  ...
  //  printf("Thread %d running RRT\n", threadIdx.x);
    ...
    for (int iter = 1; iter < maxIter && size < maxNodes; ++iter) {
        TreeNode newNode = sampleFreeSpace(&grid, iter, startX, startY, startYaw);
