...
RRTNode::RRTNode()
    : Node("rrt_node_cpp"), rng_(std::random_device{}()),
    ...
    map_sub_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        "/map", map_qos, std::bind(&RRTNode::mapCallback, this, std::placeholders::_1));

    planning_timer_ =
        this->create_wall_timer(100ms, std::bind(&RRTNode::planningTimerCallback, this));
...
RRTNode::sample(const std::optional<std::array<double, 2>> &goal, int iteration) {
    if (!current_pose_) {
        return std::nullopt;
    }
...
void RRTNode::visualize(const std::vector<TreeNode> &tree,
                        const std::vector<TreeNode> *path) const {
    if (!tree_pub_) {
        return;
    }
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<dynamic_rrt::RRTNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
