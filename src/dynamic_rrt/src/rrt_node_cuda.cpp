...
#if DYNAMIC_RRT_HAS_CUDA
#include "cuda/cuda_rrt.hpp"
#endif
...
RRTNodeCuda::RRTNodeCuda()
    : Node("rrt_node_cuda"), rng_(std::random_device{}()),
    ...
    const std::string pose_topic_param =
        this->declare_parameter<std::string>("pose_topic", "/pf/viz/inferred_pose");
    const std::string odom_topic_param =
        this->declare_parameter<std::string>("odom_topic", "/ego_racecar/odom");
    const int declared_max_iterations =
        this->declare_parameter<int>("max_iterations", max_iterations_);
    max_iterations_ = std::max(1, declared_max_iterations);
    goal_lookahead_distance_ =
        this->declare_parameter<double>("goal_lookahead_distance", goal_lookahead_distance_);
    goal_bias_probability_ =
        this->declare_parameter<double>("goal_bias_probability", goal_bias_probability_);
    goal_bias_probability_ = std::clamp(goal_bias_probability_, 0.0, 1.0);
    const int declared_goal_bias_start_iteration =
        this->declare_parameter<int>("goal_bias_start_iteration", goal_bias_start_iteration_);
    goal_bias_start_iteration_ = std::max(0, declared_goal_bias_start_iteration);
    goal_bias_longitudinal_stddev_ =
        std::max(0.01, this->declare_parameter<double>("goal_bias_longitudinal_stddev",
                                                       goal_bias_longitudinal_stddev_));
    goal_bias_lateral_stddev_ =
        std::max(0.01, this->declare_parameter<double>("goal_bias_lateral_stddev",
                                                       goal_bias_lateral_stddev_));
    sample_forward_min_ =
        std::max(0.01, this->declare_parameter<double>("sample_forward_min", sample_forward_min_));
    sample_forward_max_ =
        this->declare_parameter<double>("sample_forward_max", sample_forward_max_);
    if (sample_forward_max_ <= sample_forward_min_ + 0.05) {
        sample_forward_max_ = sample_forward_min_ + 0.05;
    }
    sample_lateral_range_ = std::max(
        0.05, this->declare_parameter<double>("sample_lateral_range", sample_lateral_range_));

    forward_dist_ =
        std::uniform_real_distribution<double>(sample_forward_min_, sample_forward_max_);
    lateral_dist_ =
        std::uniform_real_distribution<double>(-sample_lateral_range_, sample_lateral_range_);
    ...
    waypoint_pub_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>("/waypoints_markers", 10);
    sample_pub_ =
        this->create_publisher<visualization_msgs::msg::MarkerArray>("/sample_points", 10);
    drive_pub_ =
        this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>("/drive", 10);
    ...
    if (sim_mode_) {
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            odom_topic_param, qos, std::bind(&RRTNodeCuda::odomCallback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(),
                    "Running CUDA RRT in sim mode; subscribing to Odometry on %s",
                    odom_topic_param.c_str());
    } else {
        pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            pose_topic_param, qos, std::bind(&RRTNodeCuda::poseCallback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(),
                    "Running CUDA RRT in real mode; subscribing to PoseStamped on %s",
                    pose_topic_param.c_str());
    }

    scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "/scan", qos, std::bind(&RRTNodeCuda::scanCallback, this, std::placeholders::_1));
    ...
    map_sub_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        "/map", map_qos, std::bind(&RRTNodeCuda::mapCallback, this, std::placeholders::_1));

    planning_timer_ =
        this->create_wall_timer(100ms, std::bind(&RRTNodeCuda::planningTimerCallback, this));

    if (!loadWaypoints()) {
        RCLCPP_WARN(this->get_logger(), "Failed to load waypoints; GPU planner will stay idle.");
    }
...
void RRTNodeCuda::poseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
...
void RRTNodeCuda::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
...
void RRTNodeCuda::scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
...
void RRTNodeCuda::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg) {
    map_original_ = *msg;
    map_inflated_ = *msg;
    map_frame_ = msg->header.frame_id.empty() ? std::string("map") : msg->header.frame_id;
    inflateMap(map_inflated_);
    map_ready_ = true;
    RCLCPP_INFO_ONCE(this->get_logger(), "Occupancy grid received and inflated for GPU planner.");
}

void RRTNodeCuda::planningTimerCallback() {
    ...
    if (!goal_opt) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                             "No forward waypoint available for GPU planner.");
        return;
    }

    const std::array<double, 2> start{current_pose_->x, current_pose_->y};
    const std::array<double, 2> goal{(*goal_opt)[0], (*goal_opt)[1]};

    std::vector<TreeNode> tree;
    std::vector<TreeNode> path;
    planOnGpu(start, goal, tree, path);

    const bool found = !path.empty();
    if (!found) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                             "CUDA RRT failed to find a path after %d iterations.",
                             max_iterations_);
    } else {
        path_points_.clear();
...
RRTNodeCuda::sample(const std::optional<std::array<double, 2>> &goal, int iteration) {
    if (!current_pose_) {
        return std::nullopt;
    }
...
int RRTNodeCuda::nearest(const std::vector<TreeNode> &tree, const std::array<double, 2> &point) const {
...
TreeNode RRTNodeCuda::steer(const TreeNode &nearest_node, const std::array<double, 2> &point) const {
    TreeNode result;
...
bool RRTNodeCuda::checkCollision(const TreeNode &from, const TreeNode &to) const {
...
bool RRTNodeCuda::isGoal(const TreeNode &node, double goal_x, double goal_y) const {
    const double dx = node.x - goal_x;
    const double dy = node.y - goal_y;
    return (dx * dx + dy * dy) <= (goal_threshold_ * goal_threshold_);
}

std::vector<TreeNode> RRTNodeCuda::buildPath(const std::vector<TreeNode> &tree, int goal_index) const {
    std::vector<TreeNode> path;
    int idx = goal_index;
...
std::optional<std::array<double, 2>> RRTNodeCuda::selectGoal() const {
    if (!current_pose_) {
        return std::nullopt;
    }
    ...
    for (const auto &waypoint : waypoints_) {
        const double dx = waypoint[0] - car_position[0];
        const double dy = waypoint[1] - car_position[1];

        const double forward = dx * cos_yaw + dy * sin_yaw;
        const double lateral = -dx * sin_yaw + dy * cos_yaw;

        if (forward < kMinLookahead) {
            continue;
        }

        const double distance = std::sqrt(dx * dx + dy * dy);
        const bool within_goal_window = std::abs(distance - goal_lookahead_distance_) <= 6.0;

        if (within_goal_window && std::abs(lateral) < sample_lateral_range_) {
...
void RRTNodeCuda::visualize(const std::vector<TreeNode> &tree, const std::vector<TreeNode> *path) const {
    if (!tree_pub_) {
        return;
    }
...
bool RRTNodeCuda::loadWaypoints() {
    std::string package_share;
    try {
        package_share = ament_index_cpp::get_package_share_directory("dynamic_rrt");
...
void RRTNodeCuda::inflateMap(nav_msgs::msg::OccupancyGrid &map) const {
...
bool RRTNodeCuda::isPointFree(double x, double y) const {
    if (!map_ready_ || map_inflated_.data.empty() || map_inflated_.info.resolution <= 0.0) {
        return false;
    }

    const double origin_x = map_inflated_.info.origin.position.x;
    const double origin_y = map_inflated_.info.origin.position.y;
    const double resolution = map_inflated_.info.resolution;
    const int width = static_cast<int>(map_inflated_.info.width);
    const int height = static_cast<int>(map_inflated_.info.height);
...
double RRTNodeCuda::yawFromQuaternion(double x, double y, double z, double w) {
    const double siny_cosp = 2.0 * (w * z + x * y);
    const double cosy_cosp = 1.0 - 2.0 * (y * y + z * z);
    return std::atan2(siny_cosp, cosy_cosp);
}

void RRTNodeCuda::publishWaypointsMarkers() const {
    if (!waypoint_pub_) {
        return;
    }
...
void RRTNodeCuda::publishSampleMarkers() const {
    if (!sample_pub_) {
        return;
    }
...
void RRTNodeCuda::addSamplePoint(double x, double y) {
...
void RRTNodeCuda::planOnGpu(const std::array<double, 2> &start, const std::array<double, 2> &goal,
                            std::vector<TreeNode> &tree, std::vector<TreeNode> &path) {
#if DYNAMIC_RRT_HAS_CUDA
    tree.clear();
    path.clear();
    try {
        const TreeNode start_node{start[0], start[1], -1};
        const auto gpu_result = cuda::runCudaRrt(map_inflated_, start_node, goal, max_iterations_,
                                                 steer_range_, goal_threshold_);
        tree = gpu_result.tree;
        path = gpu_result.path;
        if (!path.empty()) {
            sample_points_.clear();
            RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                                 "CUDA RRT found a path with %zu nodes.", path.size());
            return;
        }
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                              "CUDA kernel did not produce a valid path; falling back to CPU loop.");
    } catch (const std::exception &e) {
        RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                              "CUDA planning failed: %s", e.what());
    }
#endif
    ...
    tree.push_back(TreeNode{start[0], start[1], -1});

    const std::optional<std::array<double, 2>> goal_opt(goal);
    bool found = false;
    int goal_index = -1;
    ...
    for (int i = 0; i < max_iterations_; ++i) {
        ...
        if (isGoal(new_node, goal[0], goal[1])) {
            goal_index = static_cast<int>(tree.size()) - 1;
            found = true;
            RCLCPP_INFO(this->get_logger(),
                        "CUDA fallback path found after %d iterations using CPU sampling.", i + 1);
            break;
        }
    }

    if (found && goal_index >= 0) {
        path = buildPath(tree, goal_index);
    }

    if (!found) {
        RCLCPP_DEBUG(this->get_logger(),
                     "CUDA fallback exhausted iterations without finding a goal.");
    }
}
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<dynamic_rrt::RRTNodeCuda>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
